...
public class AdventureGame implements Serializable {
    ...
    public boolean movePlayer(String direction) {
        ...
        for (Passage entry : possibilities) {
            if (entry.getIsBlocked()) { // this entry requires a key, and if we have the key we must go into this entry
                if (this.player.getInventory().contains(entry.getKeyName())) { // we have the key, we must go into this entry
    ...
    public String interpretAction(String command){
        ...
        if (motionTable.optionExists(inputArray[0])) {
            if (!movePlayer(inputArray[0])) { //move the player to the corresponding direction
            ...
            if(inputArray[0].equals("QUIT")) { return "GAME OVER"; } //time to stop!
                ...
                //if the item dropped is a health potion, then the player is actually drinking it
                if(inputArray[1].equals("BANDAGE")){
                    return "BANDAGE";
                }
